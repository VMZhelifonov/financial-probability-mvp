...
def calibrate_and_simulate(
    ...
    if len(future_prices) == 0:
        return None, None, None, None, "Simulation failed"
...
if ticker:
    with st.spinner("Fetching data and running simulation..."):
        all_paths, future_prices, current_price, model_desc, error = calibrate_and_simulate(
            ticker, forecast_days, model_choice, n_paths=5000
        )

    if error:
        st.error(f"Error: {error}. Try a major ticker like AAPL, MSFT, or SPY.")
